...
export const Dashboard = () => {
    ...
    const isSharedEmailValid = async () => {
        if (sharedEmail === "") setCreateDocError("Email is required");
            ...
            try {
                ...
                const response = await axios.get(`https://api.co-write.online/api/users/find`, {
                ...
                if (response.status === 200) {
                    ...
                    await axios.post(
                        `https://api.co-write.online/api/users/allowUser`,
                        {
                            documentId: docId,
                            emailId: sharedEmail,
                            role: role,
    ...
    const getDocuments = async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.get(`https://api.co-write.online/api/users/documents`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const getUserProfile = async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.get(`https://api.co-write.online/api/users/profile`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleAuthError = async (error, fn) => {
        if (error.response?.status === 403) {
            try {
                const newToken = await axios.post(`https://api.co-write.online/auth/users/token`, {
    ...
    const createNewDoc = async () => {
        try {
            ...
            const response = await axios.post(
                `https://api.co-write.online/api/users/createNewDoc`,
    ...
    const deleteDoc = async (documentName) => {
        try {
            const token = localStorage.getItem("token");
            await axios.delete(`https://api.co-write.online/api/users/documents/del`, {
                headers: { Authorization: `Bearer ${token}` },
    ...
    const renameDoc = async () => {
        try {
            const token = localStorage.getItem("token");
            await axios.put(
                `https://api.co-write.online/api/users/documents/rename`,
    ...
    const logout = async () => {
        await axios.delete(`https://api.co-write.online/auth/users/logout`, {
