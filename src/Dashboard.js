...
export const Dashboard = () => {
    ...
    const isSharedEmailValid = async () => {
        if (sharedEmail === "") setCreateDocError("Email is required");
            ...
            try {
                ...
                const response = await axios.get("http://18.119.123.153/api/users/find", {
                ...
                if (response.status === 200) {
                    ...
                    await axios.post(
                        "http://18.119.123.153/api/users/allowUser",
                        {
                            documentId: docId,
                            emailId: sharedEmail,
                            role: role,
    ...
    const getDocuments = async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.get("http://18.119.123.153/api/users/documents", {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const getUserProfile = async () => {
        try {
            const token = localStorage.getItem("token");
            const response = await axios.get("http://18.119.123.153/api/users/profile", {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleAuthError = async (error, fn) => {
        if (error.response?.status === 403) {
            try {
                const newToken = await axios.post("http://18.119.123.153/auth/users/token", {
    ...
    const createNewDoc = async () => {
        try {
            ...
            const response = await axios.post(
                "http://18.119.123.153/api/users/createNewDoc",
    ...
    const deleteDoc = async (documentName) => {
        try {
            const token = localStorage.getItem("token");
            await axios.delete("http://18.119.123.153/api/users/documents/del", {
                headers: { Authorization: `Bearer ${token}` },
    ...
    const renameDoc = async () => {
        try {
            const token = localStorage.getItem("token");
            await axios.put(
                "http://18.119.123.153/api/users/documents/rename",
    ...
    const logout = async () => {
        await axios.delete("http://18.119.123.153/auth/users/logout", {
